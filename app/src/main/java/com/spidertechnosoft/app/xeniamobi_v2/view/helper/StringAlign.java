package com.spidertechnosoft.app.xeniamobi_v2.view.helper;
...
public class StringAlign {
     ...
     *            待对齐的字符串
     */
    public String format(String s,String delimiter) {
        ...
        switch (this.just) {
            case JUST_RIGHT:
                ...
                append(where, maxChars - wantedLength,delimiter);
                // 将字符串添加在右边
                where.append(wanted);
                break;
            case JUST_CENTER:
                ...
                append(where, (maxChars - wantedLength) / 2,delimiter);
                where.append(wanted);
                append(where, (maxChars - wantedLength) / 2,delimiter);
                // 调整舍入误差
                append(where, maxChars - (where.length() - startPos),delimiter);
                break;
            case JUST_LEFT:
                // 右对齐，将空格字符放在字符串右边。
                where.append(wanted);
        ...
        if (s.length() > wantedLength) {
            ...
            where.append("\n" + this.format(remainStr,delimiter));
        }
        return where.toString();
    }

    protected final void append(StringBuffer to, int howMany,String delimiter) {

        if(delimiter==null)
            pad(to, howMany);
        else
            pad(to, howMany,delimiter);
    }
    ...
    protected final void pad(StringBuffer to, int howMany,String delimiter) {
        for (int i = 0; i < howMany; i++)
            to.append(delimiter);
    }
